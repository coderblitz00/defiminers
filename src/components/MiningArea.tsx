...
export const MiningArea = ({ miners, ores }: MiningAreaProps) => {
  ...
  const renderCanvas = () => {
    ...
    ores.forEach(ore => {
      ...
      switch(ore.type) {
        case 'coal': oreColor = '#1f2937'; break;
        case 'iron': oreColor = '#6b7280'; break;
        case 'copper': oreColor = '#d97706'; break;
        case 'gold': oreColor = '#fbbf24'; break;
        case 'crystal': oreColor = '#0ea5e9'; break;
        case 'gem': oreColor = '#8b5cf6'; break;
        case 'legendary': oreColor = '#ef4444'; break;
        case 'tin': oreColor = '#94a3b8'; break;
        case 'silver': oreColor = '#cbd5e1'; break;
        case 'mithril': oreColor = '#6366f1'; break;
        case 'thorium': oreColor = '#10b981'; break;
        case 'platinum': oreColor = '#e2e8f0'; break;
        case 'orichalcum': oreColor = '#f97316'; break;
        case 'uranium': oreColor = '#84cc16'; break;
      }
    ...
    return (
      <div className="pixel-container glass-panel relative w-full h-full rounded-lg overflow-hidden animate-fade-in shadow-xl border border-white/10 flex flex-col items-center justify-center gap-4">
        <div className="text-center">
          <p className="text-white mb-2">Loading mining graphics... {Math.round(loadingProgress)}%</p>
          <div className="w-48 h-2 bg-gray-700 rounded-full overflow-hidden">
            <Progress value={loadingProgress} className="h-full" />
          </div>
        </div>
        <p className="text-xs text-gray-400 max-w-xs text-center">
          {loadingError || "Using stylized pixel graphics for mining simulation..."}
        </p>
      </div>
    );
  }
