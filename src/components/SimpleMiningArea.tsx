...
export const SimpleMiningArea = ({ miners, ores, activeMine = 'starter' }: SimpleMiningAreaProps) => {
  ...
  const getOreColor = (type: OreType) => {
    switch(type) {
      ...
      case 'crystal': return '#0ea5e9';
      case 'gem': return '#8b5cf6';
      case 'legendary': return '#ef4444';
      case 'tin': return '#94a3b8';
      case 'silver': return '#cbd5e1';
      case 'mithril': return '#6366f1';
      case 'thorium': return '#10b981';
      case 'platinum': return '#e2e8f0';
      case 'orichalcum': return '#f97316';
  ...
  const getMinerColor = (type: string) => {
    switch (type) {
      case 'basic': return '#60a5fa'; // blue
      case 'expert': return '#f59e0b'; // amber
      case 'hauler': return '#10b981'; // emerald
      case 'prospector': return '#8b5cf6'; // violet
      case 'engineer': return '#ef4444'; // red
  ...
  const getMinerStatusBadge = (miner: Miner) => {
    ...
    if (miner.state === 'mining') {
      ...
      return <Badge variant="resting">Dropping off ({Math.ceil(miner.restDuration || 5) - Math.floor(miner.restProgress || 0)}s)</Badge>;
    }
    return null;
  };
    ...
    <div className="relative w-full h-full bg-emerald-700 rounded-lg overflow-hidden border border-emerald-900">
      ...
      <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 gap-1 p-1">
        ...
        {[...Array(9)].map((_, index) => (
          ...
          >
            {index === 4 && <span className="text-xs text-emerald-100 opacity-70">Mining Area</span>}
          </div>
        ))}
      </div>
      ...
      {/* Render ores as colored circles with ore type labels */}
      {ores.map(ore => (
        <div 
          key={ore.id}
          className="absolute w-4 h-4 rounded-full border border-white/20 transition-all"
          style={{ 
            left: `${ore.position.x}%`, 
            top: `${ore.position.y}%`, 
            ...
            transform: 'translate(-50%, -50%)',
            boxShadow: `0 0 ${oreData[ore.type].value / 5}px ${getOreColor(ore.type)}`
          }}
        >
      ...
      {miners.map(miner => {
        const isFullInventory = Object.values(miner.inventory).reduce((sum, count) => sum + count, 0) >= miner.capacity;
        
        return (
          <div 
            key={miner.id}
            className={`absolute w-5 h-5 rounded-full flex items-center justify-center text-xs text-white font-bold border-2 ${isFullInventory && miner.state !== 'resting' ? 'border-red-500 animate-pulse' : 'border-white'}`}
            style={{ 
            ...
            {miner.state === 'resting' && (
              <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 w-10 h-1 bg-black/30 rounded overflow-hidden">
                <div 
                  ...
                  style={{ width: `${((miner.restProgress || 0) / (miner.restDuration || 5)) * 100}%` }}
                />
              </div>
            )}
      ...
      <div className="absolute top-2 right-2 bg-black/50 p-1 rounded text-[8px] text-white grid grid-cols-2 gap-x-2 gap-y-1 max-w-[120px]">
        {Object.entries(oreData).slice(0, 6).map(([type, data]) => (
          <div key={type} className="flex items-center gap-1">
            <div 
              className="w-2 h-2 rounded-full" 
              style={{ backgroundColor: getOreColor(type as OreType) }}
            ></div>
            <span>{type} (${data.value})</span>
          </div>
        ))}
      </div>
    </div>
