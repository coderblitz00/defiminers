...
import { Ore, OreType } from '@/lib/ores';
...
const ResourceParticle = ({ 
  ...
  return (
    <div 
      className={`absolute px-1 py-0.5 rounded-full ${oreColors[type] || 'bg-gray-800 text-white'} text-xs resource-particle font-bold`}
      style={{ 
        left: `${position.x}%`, 
        top: `${position.y}%`,
      }}
...
export const PixiMiningArea = ({ miners, ores }: PixiMiningAreaProps) => {
  ...
  const [tileTheme, setTileTheme] = useState<'blue' | 'brown' | 'green' | 'gray'>('blue');
  ...
  useEffect(() => {
    ...
    const handleInitError = (message: string) => {
      ...
      setLoadingProgress(100);
      setTimeout(() => setLoading(false), 500);
      toast.error("Using simplified mining view due to graphics initialization error");
    };
  ...
  const renderFloorTiles = (theme: 'blue' | 'brown' | 'green' | 'gray') => {
    ...
    try {
      ...
      const worldContainer = appRef.current.stage.getChildByName('world') as PIXI.Container;
      if (!worldContainer) {
        console.error("World container not found");
        return;
      }
      
      const floorContainer = worldContainer.getChildByName('floor') as PIXI.Container;
      if (!floorContainer) {
  ...
  const renderWalls = (theme: 'blue' | 'brown' | 'green' | 'gray') => {
    ...
    try {
      ...
      const wallsContainer = worldContainer.getChildByName('walls') as PIXI.Container;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const minersContainer = worldContainer.getChildByName('miners') as PIXI.Container;
      ...
      miners.forEach(miner => {
        ...
        switch (miner.type) {
          case 'basic': color = 0x60a5fa; break; // blue
          case 'expert': color = 0xf59e0b; break; // amber
          case 'hauler': color = 0x10b981; break; // emerald
          case 'prospector': color = 0x8b5cf6; break; // violet
          case 'engineer': color = 0xef4444; break; // red
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oresContainer = worldContainer.getChildByName('ores') as PIXI.Container;
      ...
      ores.forEach(ore => {
        ...
        let color = 0x6b7280; // Default gray
        switch(ore.type) {
          case 'coal': color = 0x1f2937; break;
          case 'iron': color = 0x6b7280; break;
          case 'copper': color = 0xd97706; break;
          case 'gold': color = 0xfbbf24; break;
          case 'crystal': color = 0x0ea5e9; break;
          case 'gem': color = 0x8b5cf6; break;
          case 'legendary': color = 0xef4444; break;
          case 'tin': color = 0x94a3b8; break;
          case 'silver': color = 0xcbd5e1; break;
          case 'mithril': color = 0x6366f1; break;
          case 'thorium': color = 0x10b981; break;
          case 'platinum': color = 0xe2e8f0; break;
          case 'orichalcum': color = 0xf97316; break;
          case 'uranium': color = 0x84cc16; break;
        }
  ...
  useEffect(() => {
    ...
    const themes: Array<'blue' | 'brown' | 'green' | 'gray'> = ['blue', 'brown', 'green', 'gray'];
          ...
          <div className="grid grid-cols-4 gap-2">
            {miners.map((miner, i) => (
              <div 
                ...
                style={{
                  background: miner.type === 'basic' ? '#60a5fa' : 
                              miner.type === 'expert' ? '#f59e0b' : 
                              miner.type === 'hauler' ? '#10b981' : 
                              miner.type === 'prospector' ? '#8b5cf6' : '#ef4444'
                }}
              >
                <span className="text-xs text-white font-bold">{miner.type[0].toUpperCase()}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    return (
      <div className="pixel-container glass-panel relative w-full h-full rounded-lg overflow-hidden animate-fade-in shadow-xl border border-white/10 flex flex-col items-center justify-center gap-4">
        <div className="text-center">
          <p className="text-white mb-2">Initializing pixel world... {Math.round(loadingProgress)}%</p>
